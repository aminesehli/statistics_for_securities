...
def plot_zscore(df, stock1, stock2, ratio):
    ...
    plt.legend(['Z-score', 'Mean', '+1','+1.25','-1','-1.25'], loc='best')
    
    return zscore

def plot_moving_average_ratio(df, stock1, stock2, ratio, window1=5, window2=20):
    '''
    Plots the moving averages of the price ratio between two stocks.
    ...
    Args:
        ...
        stock2 (str): Second stock ticker name.
        ratio (pd.Series): Price ratio of the two stocks.
        window1 (int): Window size for the first moving average.
        window2 (int): Window size for the second moving average.
    
    Returns:
        zscore_ma (pd.Series): Z-score of the moving averages of the price ratio.
    '''
    ratio_mavg1 = ratio.rolling(window=window1, center=False).mean()
    ratio_mavg2 = ratio.rolling(window=window2, center=False).mean()
    std = ratio.rolling(window=window2, center=False).std()
    zscore_ma = (ratio_mavg1 - ratio_mavg2)/std
    plt.figure(figsize=(10,6))
    plt.plot(ratio.index, ratio.values)
    plt.plot(ratio_mavg1.index, ratio_mavg1.values)
    plt.plot(ratio_mavg2.index, ratio_mavg2.values)
    plt.legend(['Ratio', f"{window1} Ratio MA", f"{window2} Ratio MA"])
    plt.ylabel('Ratio')
    plt.xlabel('Date')
    plt.title(f"{stock1} and {stock2} Ratio with Moving Averages")
    
    return zscore_ma

def plot_rolling_zscore(df, stock1, stock2, zscore):
    '''
    Plots the rolling Z-score of the price ratio between two stocks.
    
    Args:
        df (pd.DataFrame): DataFrame containing the closing prices of stocks.
        stock1 (str): First stock ticker name.
        stock2 (str): Second stock ticker name.
        zscore (pd.Series): Z-score of the moving averages of the price ratio.
    ...
    plt.ylabel('Z-score')
    plt.xlabel('Date')
    plt.title(f"Rolling Z-score of ratio of {stock1} and {stock2}")

def plot_basic_signals(df, stock1, stock2, zscore, ratio):
...
zscore_ma = plot_moving_average_ratio(dataClose, selected_tickers[0], selected_tickers[1], priceRatio)

plot_rolling_zscore(dataClose, selected_tickers[0], selected_tickers[1], zscore_ma)
plot_basic_signals(dataClose, selected_tickers[0], selected_tickers[1], zscore_ma, priceRatio)
